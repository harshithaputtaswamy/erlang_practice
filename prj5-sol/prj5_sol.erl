...
rec_poly_eval([], _, Sum, _) ->
    Sum;
rec_poly_eval([Coeff | Rest], X, Sum, Power) ->
    NewSum = Sum + Coeff * math:pow(X, Power),
    rec_poly_eval(Rest, X, NewSum, Power + 1).
...
tuple_poly_eval(TupleCoeffs, X) ->
    tuple_poly_eval(TupleCoeffs, X, 0, 0).

tuple_poly_eval([], _X, _Power, Acc) ->
    Acc;
tuple_poly_eval([{_, Coeff} | Rest], X, Power, Acc) ->
    tuple_poly_eval(Rest, X, Power + 1, Acc + Coeff * math:pow(X, Power)).
...
server_fn(Assoc, Coeffs) ->
    receive
        ...
        {ClientPid, eval, X} -> 
            Result = round(id_poly_eval(Assoc, Coeffs, X)),
            ClientPid ! {self(), eval, Result}, 
            server_fn(Assoc, Coeffs); 
        OtherMsg -> 
            io:format(standard_error, "unknown message ~p~n", [OtherMsg]),
            server_fn(Assoc, Coeffs)  
     end.
